...
import { Clock, AlertCircle, CheckCircle2, PlayCircle, Loader2, Upload, FileCheck, Trash2, Link2, ListChecks, Presentation, ShieldAlert, ShieldX } from 'lucide-react';
...
export default function TestPage() {
    ...
    const violationReportingRef = useRef(false);
    ...
    // Only active for non-presentation tests that are in progress
    // ────────────────────────────────────────────────
    useEffect(() => {
        if (!started || isSubmitting || isTestTerminated) return;
        if (testData?.test.test_type === 'presentation') return;

        const reportViolation = async (type: string) => {
            // Prevent duplicate reports from multiple events firing together
            if (violationReportingRef.current) return;
            violationReportingRef.current = true;

            try {
                const res = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL || ''}/api/public/tests/${token}/report-violation`,
                    {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                ...
                if (res.ok) {
                    ...
                    setViolationCount(data.tab_switches);
                    if (data.terminated) {
                ...
                // Debounce: wait 2s before allowing another report
                setTimeout(() => {
                    violationReportingRef.current = false;
                }, 2000);
            }
        };
        ...
        const handleWindowBlur = () => {
            // Only fire if the document is also about to become hidden,
            // or if it's a genuine app switch (mobile case)
            // Use a small delay to avoid double-counting with visibilitychange
            setTimeout(() => {
    ...
    }, [started, isSubmitting, isTestTerminated, testData, token]);

    // Anti-Cheat: Block right-click and keyboard shortcuts
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            // Block F12, Ctrl+Shift+I (Dev Tools), Ctrl+U (View Source)
            if (
                e.key === 'F12' ||
    ...
    return (
        <>
            ...
            {/* Anti-Cheat: Violation Warning Dialog */}
            {showViolationWarning && !isTestTerminated && (
                <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <Card className="max-w-lg w-full border-2 border-amber-500 shadow-2xl shadow-amber-500/20 animate-in zoom-in-95 duration-300">
                        <CardHeader className="bg-amber-50 dark:bg-amber-900/30 text-center pb-4">
                            <div className="mx-auto h-16 w-16 rounded-full bg-amber-100 dark:bg-amber-900/50 flex items-center justify-center mb-3">
                                <ShieldAlert className="h-8 w-8 text-amber-600 dark:text-amber-400" />
                            </div>
                            <CardTitle className="text-xl text-amber-700 dark:text-amber-300">
                                {t('test.anticheat.warning_title')}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="pt-6 space-y-4">
                            <p className="text-sm text-muted-foreground leading-relaxed text-center">
                                {t('test.anticheat.warning_desc')}
                            </p>
                            <div className="text-center p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                                <p className="text-sm font-bold text-red-600 dark:text-red-400">
                                    {t('test.anticheat.warning_strikes').replace('{remaining}', String(Math.max(0, 2 - violationCount)))}
                                </p>
                            </div>
                            <Button
                                size="lg"
            ...
            {/* Anti-Cheat: Test Terminated Overlay */}
            {isTestTerminated && (
                <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-500">
                    <Card className="max-w-lg w-full border-2 border-red-500 shadow-2xl shadow-red-500/30 animate-in zoom-in-95 duration-300">
                        <CardHeader className="bg-red-50 dark:bg-red-900/30 text-center pb-4">
                            <div className="mx-auto h-16 w-16 rounded-full bg-red-100 dark:bg-red-900/50 flex items-center justify-center mb-3">
                                <ShieldX className="h-8 w-8 text-red-600 dark:text-red-400" />
                            </div>
                            <CardTitle className="text-xl text-red-700 dark:text-red-300">
                                {t('test.anticheat.terminated_title')}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="pt-6 space-y-4">
                            <p className="text-sm text-muted-foreground leading-relaxed text-center">
                                {t('test.anticheat.terminated_desc')}
                            </p>
                            <Button
                                size="lg"
                                variant="destructive"
                                className="w-full h-12 text-base font-semibold"
                                onClick={() => router.push(`/test/${token}/result`)}
                            >
                                {t('test.anticheat.terminated_btn')}
                            </Button>
                        </CardContent>
                    </Card>
                </div>
